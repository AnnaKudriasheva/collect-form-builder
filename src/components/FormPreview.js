  ...
  return (
    <>
      <h3 style={{ marginBottom: '2rem', textAlign: 'center' }}>
        Form Preview
        <Button 
          style={{ borderRadius: '50%', marginLeft: '10px', width: '30px', height: '30px', padding: '0'}} 
          type="default"
          onClick={refreshForm}
        >
          <SyncOutlined />
        ...
        <Form>
          {
            nodes.map((field, idx) => (
              <>
                ...
                <div id={field.split(' ').join('-').toLowerCase()} className="mb-1" style={styles.wrapper} key={idx}></div>
              </>
            ))
          }
        </Form> :
